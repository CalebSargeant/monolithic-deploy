...
if git diff --cached --name-only >/dev/null 2>&1; then
    DOCKER_FILES=$(git diff --cached --name-only | grep -E "(Dockerfile.*|docker-compose.*\.ya?ml|\.dockerignore)" || true)
    CONTEXT="staged"
else
...
run_security_tool() {
    ...
    if command -v "$(echo $command | awk '{print $1}')" &> /dev/null; then
        ...
        echo -e "${YELLOW}⚠️  ${tool_name} not available - install with: brew install $(echo $command | awk '{print $1}')${NC}"
    fi
}
