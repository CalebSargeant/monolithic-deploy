...
if [ -n "$PYTHON_FILES" ]; then
    ...
    run_security_tool "Bandit" \
        "bandit -r . -x tests/ -f json -o /tmp/bandit-report.json && bandit -r . -x tests/ || true" \
    ...
    run_security_tool "Semgrep" \
        "semgrep --config=auto --json --output=/tmp/semgrep-report.json . || true && semgrep --config=auto . || true" \
...
if [ -n "$JS_FILES" ]; then
    ...
    run_security_tool "ESLint Security" \
        "npx eslint --ext .js,.ts,.jsx,.tsx . --config .eslintrc.security.js || true" \
